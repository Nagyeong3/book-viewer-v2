...
import { createElement, FormEvent, PointerEvent as ReactPointerEvent, useEffect, useMemo, useRef, useState } from "react";
...
function SemanticBlock({ item }: { item: ContentItem }) {
  ...
  if (item.content_type === "image") {
    const imageUrl = getSafeImageUrl(item.cropped_image_path);
    if (imageUrl) {
      return (
        <figure className="coordinate-image-frame">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img className="coordinate-cropped-image" src={imageUrl} alt={text || `content ${item.id} image`} />
          {text ? <figcaption>{text}</figcaption> : null}
        </figure>
      );
    }
    return <div className="coordinate-placeholder image">이미지 경로 없음</div>;
  }

  if (item.content_type === "table") {
    return <div className="coordinate-placeholder table">{text || "표 영역"}</div>;
  }
...
function CoordinatePage({ contents, focusContentId }: { contents: ContentItem[]; focusContentId: number | null }) {
  ...
  if (!positioned.length) return <div className="coordinate-empty">이 페이지에는 좌표 데이터가 없습니다.</div>;
