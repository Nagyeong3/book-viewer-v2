...
export default function WorkspaceV3() {
  ...
  return (
    <main className="workspace-shell workspace-v3">
      <header className="topbar product-topbar"><div className="brand-block"><span className="brand-mark" aria-hidden="true">B</span><span className="brand-copy"><strong>기술문서 AI Viewer</strong><small>{activeDocument?.title ?? "문서를 선택하세요"}</small></span></div><div className="topbar-actions">{chapter ? <span className="chapter-pill"><span>현재 위치</span>{chapter.title} · p.{activePage ?? chapter.startPage}</span> : null}{layout.chatCollapsed ? <button type="button" className="topbar-ghost" onClick={() => setLayout((current) => ({ ...current, chatCollapsed: false }))}><UiIcon name="sparkles" size={12} /><span>AI 비서 열기</span></button> : null}<div className="scope-search-control"><UiIcon name="search" size={13} /><input value={scopeSearchQuery} onChange={(event) => setScopeSearchQuery(event.target.value)} placeholder={selectedIds.length ? "선택 문서 검색" : "검색 문서를 선택하세요"} disabled={!selectedIds.length} />{scopeSearchQuery.trim().length >= 2 ? <div className="scope-search-results">{scopeSearchResults.length ? scopeSearchResults.map((result) => <button key={`${result.document.id}-${result.item.id}`} type="button" onClick={() => openScopeSearchResult(result)}><span><strong>{result.item.text?.replace(/<[^>]*>/g, "").slice(0, 72) || `Content #${result.item.id}`}</strong><small>{result.document.title}</small></span><em>p.{result.item.page ?? "-"}</em></button>) : <div className="scope-search-empty">선택한 문서에서 결과를 찾지 못했습니다.</div>}</div> : null}</div><span className="topbar-index-slot" /></div></header>
