...
export default function WorkspaceV3() {
  ...
  const effectiveTranslationLanguage =
    translationLanguage === CUSTOM_TRANSLATION_LANGUAGE ? customTranslationLanguage.trim() : translationLanguage;
  const partialTranslationReady = translationEnabled && effectiveTranslationLanguage.length >= 2;
  ...
  async function translateItem(item: ContentItem) {
    ...
    if (!partialTranslationReady || !source) return;
    const key = `${effectiveTranslationLanguage}:${item.id}:${source}`;
    ...
    try {
      const result = await api.translate(source, effectiveTranslationLanguage);
  ...
  return (
    <main className="workspace-shell workspace-v3">
      ...
      <section className="workspace-grid" style={gridStyle}>
        ...
        <section className="viewer-pane panel chapter-viewer-panel"><div className="panel-heading viewer-heading sticky-viewer-heading"><div className="viewer-heading-main"><button type="button" className="viewer-menu-button" aria-label={layout.tocCollapsed ? "목차 열기" : "목차 닫기"} title={layout.tocCollapsed ? "목차 열기" : "목차 닫기"} onClick={() => setLayout((current) => ({ ...current, tocCollapsed: !current.tocCollapsed }))}><span /><span /><span /></button><div><span className="panel-eyebrow">DOCUMENT VIEWER</span><h2>{activeDocument?.title ?? "문서 선택"}</h2><p>{chapter ? `${chapter.title} · p.${chapter.startPage}–${chapter.endPage}` : "챕터를 불러오는 중"}</p></div></div><div className="viewer-toolbar">
          <div className="translate-controls full-translate-controls">
            <button
              type="button"
              className={`translate-toggle ${fullTranslationEnabled ? "active" : ""}`}
              aria-pressed={fullTranslationEnabled}
              disabled={fullTranslationLanguagesLoading || !fullTranslationLanguages.length}
              title={!fullTranslationLanguages.length && !fullTranslationLanguagesLoading ? "이 문서에 저장된 번역문이 없습니다." : undefined}
              onClick={() => {
                setFullTranslationEnabled((current) => !current);
                setTranslationEnabled(false);
                setTranslationPanel(null);
              }}
            >
              <UiIcon name="translate" size={13} /><span>전체 번역</span>
            </button>
            <select
              value={fullTranslationLanguage}
              onChange={(event) => setFullTranslationLanguage(event.target.value)}
              aria-label="전체 번역 언어"
              disabled={fullTranslationLanguagesLoading || !fullTranslationLanguages.length}
            >
              {fullTranslationLanguagesLoading ? <option value="">확인 중...</option> : null}
              {!fullTranslationLanguagesLoading && !fullTranslationLanguages.length ? <option value="">번역문 없음</option> : null}
              {fullTranslationLanguages.map((code) => <option key={code} value={code}>{languageLabel(code)}</option>)}
            </select>
          </div>
          <div className="translate-controls partial-translate-controls">
            <button
              type="button"
              className={`translate-toggle ${translationEnabled ? "active" : ""}`}
            ...
            >
              <UiIcon name="translate" size={13} /><span>부분 번역</span>
            </button>
            {translationEnabled ? (
              <>
                <select
