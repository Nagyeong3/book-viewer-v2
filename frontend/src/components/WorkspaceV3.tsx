...
export default function WorkspaceV3() {
  ...
  return (
    <main className="workspace-shell workspace-v3">
      <header className="topbar product-topbar"><div className="brand-block"><span className="brand-mark" aria-hidden="true">B</span><span className="brand-copy"><strong>기술문서 AI Viewer</strong><small>{activeDocument?.title ?? "문서를 선택하세요"}</small></span></div><div className="topbar-actions">{chapter ? <span className="chapter-pill"><span>현재 위치</span>{chapter.title} · p.{activePage ?? chapter.startPage}</span> : null}{layout.chatCollapsed ? <button type="button" className="topbar-ghost" onClick={() => setLayout((current) => ({ ...current, chatCollapsed: false }))}>AI 비서 열기</button> : null}<span className="status-pill"><span className="status-dot" />AI 검색 {selectedIds.length}개</span></div></header>
