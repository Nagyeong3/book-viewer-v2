...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="End-to-end agentic RAG smoke test")
...
async def main() -> int:
    ...
    try:
        ...
        print(
            f"agent=ok tool={result.plan.tool} query={result.plan.query!r} "
            f"top_k={result.plan.top_k} sources={len(result.sources)}"
        )
        print("rationale=" + result.plan.rationale)
